...
def _preset_response(presets):
    ...
    mock.json.return_value = [
        {"code": 0, "initial": {"PtzPreset": presets}}
    ]
...
def test_get_preset_id_by_name():
    ...
    with patch("src.camera.requests.post", return_value=_preset_response(presets)):
...
def test_goto_preset_sends_correct_request():
    ...
    assert "PtzCtrl" in str(call_kwargs)
    assert "ToPos" in str(call_kwargs)
